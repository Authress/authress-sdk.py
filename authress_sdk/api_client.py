...
class ApiClient(object):
    ...
    def set_token(self, token):
        self.default_headers[
            "Authorization"
        ] = f'Bearer {token.replace("Bearer", "").strip()}'
    ...
    def get_client_token(self):
        ...
        try:
            current_token_is_valid = (
                ...
                and jwt.decode(self.token, options={"verify_signature": False})["exp"]
                > (
        ...
        issuer_origin = (
            ...
            if (self.host == None or self.host == "" or ".authress.io" in self.host)
            else self.host
        )
        payload = {
        ...
        jwt_token = jwt.encode(
            payload,
            decoded_access_key["privateKey"],
            algorithm=algorithm,
            headers={"kid": decoded_access_key["keyId"]},
        )
        self.token = (
            jwt_token if isinstance(jwt_token, str) else jwt_token.decode("utf-8")
        )
        return self.token
